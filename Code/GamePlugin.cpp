...
void CGamePlugin::IterateOverPlayers(std::function<void(PlayerComponent& player)> func) const
{
	for (const std::pair<int, EntityId>& playerPair : m_players)
	{
		...
		{
			if (PlayerComponent* pPlayer = pPlayerEntity->GetComponent<PlayerComponent>())
			{
				func(*pPlayer);
			}
		}
